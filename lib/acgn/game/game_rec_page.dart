...
class GameRecPage extends StatefulWidget {
  GameRecPage({this.linkModel, this.isShow = false});
  final bool isShow;

  final LinkModel? linkModel;

  @override
  State<GameRecPage> createState() => _GameRecPageState();
}

class _GameRecPageState extends State<GameRecPage> {
  bool isHud = true;
  bool netError = false;
  bool noMore = false;
  int page = 1;
  ...
  @override
  void didUpdateWidget(covariant GameRecPage oldWidget) {
  ...
  Future<bool> getData() async {
    dynamic res = await reqGameRec(id: _linkModel.id ?? 0, page: page);
    ...
    List tp = List.from(res?.data?['games'] ?? []);
    if (page == 1) {
                    ...
                    body: PullRefresh(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return GameRecCard(context: context, data: e);
                        },
                      ),
                    ),
                  );
...
class GameRecCard extends StatefulWidget {
  GameRecCard({super.key, required this.context, this.data});
  dynamic data;
  BuildContext context;

  @override
  State<GameRecCard> createState() => _GameRecCardState();
}

class _GameRecCardState extends State<GameRecCard> {
  int page = 1;
  ...
  Future<void> _getData() async {
    ...
    dynamic t =
        await reqMoreGames(sort: data["value"], page: page, limit: limit);
  ...
  Widget constructWidget(BuildContext context, dynamic e) {
    ...
    return Column(
      children: [
        Container(
          ...
          child: Row(
            ...
            children: [
              ...
              GestureDetector(
                  onTap: () {
                    Utils.navTo(context, '/gamemorepage', extra: {
                      "title": e['title'],
                      "id": e['id'],
                      "value": e['value'],
                    });
        ...
        array.isEmpty
            ? SizedBox()
            : Utils.gameListView(context, array,
                scrollEnable: false, useHoriMargin: false),
        SizedBox(height: 6.w),
        Row(
          children: [
            Expanded(
