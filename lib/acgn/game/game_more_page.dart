...
class GameMorePage extends BaseWidget {
  GameMorePage({this.param, this.sort = ""});
  String sort;
  ...
  State<StatefulWidget> cState() {
    ...
    return _GameMorePageState();
  }
}

class _GameMorePageState extends BaseWidgetState<GameMorePage> {
...
class GameMorePageChild extends StatefulWidget {
  GameMorePageChild({this.param, this.bannerFunc, this.sort = ""});
  String sort;
  final dynamic param;
  final Function(dynamic)? bannerFunc;

  @override
  State<GameMorePageChild> createState() => _GameMorePageChildState();
}

class _GameMorePageChildState extends State<GameMorePageChild> {
  int page = 1;
  bool isHud = true;
  List<dynamic> values = [];
  dynamic value;
  ...
  Future<bool> _getData() async {
    dynamic t = await (widget.sort.isNotEmpty
        ? reqGamesSort(sort: widget.sort, page: page)
        : reqMoreGames(sort: widget.param["value"], page: page));
    if (t.status != 1) {
      netWorkErr = true;
