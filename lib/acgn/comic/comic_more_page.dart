...
class ComicMorePage extends BaseWidget {
  ComicMorePage({this.param, this.sort = ""});
  String sort;
  ...
  State<StatefulWidget> cState() {
    ...
    return _ComicMorePageState();
  }
}

class _ComicMorePageState extends BaseWidgetState<ComicMorePage> {
...
class ComicMorePageChild extends StatefulWidget {
  ComicMorePageChild({this.param, this.bannerFunc, this.sort = ""});
  String sort;
  final dynamic param;
  final Function(dynamic)? bannerFunc;

  @override
  State<ComicMorePageChild> createState() => _ComicMorePageChildState();
}

class _ComicMorePageChildState extends State<ComicMorePageChild> {
  int page = 1;
  bool isHud = true;
  List<dynamic> values = [];
  dynamic value;
  ...
  Future<bool> _getData() async {
    dynamic t = await (widget.sort.isNotEmpty
        ? reqComicsSort(sort: widget.sort, page: page)
        : reqMoreComics(sort: widget.param["value"], page: page));
    if (t.status != 1) {
      netWorkErr = true;
  ...
  _comicList() {
    return Utils.comicListView(context, values);
  }
