...
class ComicCategoriesPage extends BaseWidget {
  ComicCategoriesPage({Key? key}) : super(key: key);
  @override
  State<StatefulWidget> cState() {
    // TODO: implement cState
    return _ComicCategoriesPageState();
  }
}

class _ComicCategoriesPageState extends BaseWidgetState<ComicCategoriesPage> {
  int page = 1;
  bool noMore = false;
  bool netError = false;
  bool isHud = false;
  ...
  Future<bool> getData() {
   return reqComicType(options: _filterTempMap, page: page).then((value) {
      if (value?.data == null) {
        netError = true;
        setState(() {});
        return false;
  ...
  void onCreate() {
    ...
    options =
        Provider.of<BaseStore>(context, listen: false).conf?.comic_type_nav ??
            [];
    setAppTitle(
