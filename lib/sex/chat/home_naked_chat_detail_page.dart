...
class HomeNakedChatDetailPage extends BaseWidget {
  HomeNakedChatDetailPage({Key? key, this.id = "0"}) : super(key: key);
  final String id;

  @override
  State<StatefulWidget> cState() {
    // TODO: implement cState
    return _HomeNakedChatDetailPageState();
  }
}

class _HomeNakedChatDetailPageState
    extends BaseWidgetState<HomeNakedChatDetailPage> {
  bool isHud = true;
  bool netError = false;
  bool noMore = false;
  ...
  void getData() {
    reqNakedChatDetail(id: widget.id).then((value) {
      if (value?.data == null) {
        netError = true;
        if (mounted) setState(() {});
        return;
...
//点赞
  void postLikeData() {
    reqUserLike(type: 20, id: int.parse(widget.id)).then((value) {
      if (value?.status == 1) {
...
//收藏
  void postCollectData() {
    reqUserFavorite(type: 20, id: int.parse(widget.id)).then((value) {
      if (value?.status == 1) {
  ...
  void buyGirl() {
    Utils.showAlertBuy(context, data,
        coinTip: Utils.txt('lxfsys').replaceAll('a', '${data['coins']}'),
        vipTip: Utils.txt('lxfsysvip'),
        cancelQuit: false,
        buyFunc: reqChatBuy, doneFunc: (resData) {
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                                  ...
                                  children: [
                                    Expanded(
                                      child: Text(
                                        data['name'],
                                        style:
                                            StyleTheme.font_black_7716_18_blod,
                                        maxLines: 99,
                                      ),
                                    ),
                                    Row(
                              ...
                              Text(
                                (data['pay_ct'].toString()) + Utils.txt('rlg'),
