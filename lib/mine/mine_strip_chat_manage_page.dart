...
class MineStripChatManagePage extends BaseWidget {
  MineStripChatManagePage({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> cState() {
    // TODO: implement cState
    return _MineStripChatManagePageState();
  }
}

class _MineStripChatManagePageState
    extends BaseWidgetState<MineStripChatManagePage> {
  List dataList = [];
  bool isHud = true;
  ...
  void onCreate() {
    ...
    setAppTitle(
      titleW: Text(Utils.txt('llgl'), style: StyleTheme.nav_title_font),
      rightW: user?.agent != 1
          ? null
          : GestureDetector(
...
class MyStripChatListPage extends StatefulWidget {
  const MyStripChatListPage({super.key});

  @override
  State<MyStripChatListPage> createState() => _MyStripChatListPageState();
}

class _MyStripChatListPageState extends State<MyStripChatListPage> {
  List<dynamic> navs = [];
  ...
  Widget build(BuildContext context) {
    return GenCustomNav(
      ...
      pages: navs.asMap().keys.map((e) {
        return MyStripChatListChildPage(
...
class MyStripChatListChildPage extends StatefulWidget {
  MyStripChatListChildPage({
    Key? key,
    this.status = 1,
  }) : super(key: key);
  final int status;
  ...
  @override
  State<MyStripChatListChildPage> createState() =>
      _MyStripChatListChildPageState();
}

class _MyStripChatListChildPageState extends State<MyStripChatListChildPage> {
  int page = 1;
  bool noMore = false;
  bool netError = false;
  bool isHud = true;
  ...
  Future<bool> getData() {
   return reqChatManageList(status: widget.status, page: page).then((value) {
      if (value?.data == null) {
        netError = true;
        setState(() {});
        return false;
