...
class MineDownloadVideoPage extends StatefulWidget {
  ...
  @override
  State<MineDownloadVideoPage> createState() => MineDownloadVideoPageState();
}

class MineDownloadVideoPageState extends State<MineDownloadVideoPage> {
  List<dynamic> videos = [];
  bool loading = true;
  bool isEdit = false;
  List chooseList = [];
  ...
  void onDelete() async {
    ...
    for (var i = 0; i < chooseList.length; i++) {
      if (chooseList[i]["choosed"] == true) {
        DownloadUtils.removeTask(chooseList[i]["id"]);
    ...
    box.put("download_video_tasks", videos);
    chooseList = [];
    // isEdit = false;
    // setupNav();
    if (mounted) setState(() {});
  ...
  @override
  void didUpdateWidget(covariant MineDownloadVideoPage oldWidget) {
    ...
    return GestureDetector(
      ...
      onTap: () {
        if (progress.toInt() == 1) {
          ...
          Utils.navTo(context, "/minelocalvideopage");
        } else if (!downloading && !isWaiting) {
          DownloadUtils.createDownloadTask(widget.data);
        }
      },
      child: Container(
