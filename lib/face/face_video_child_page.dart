...
class FaceVideoChildPage extends StatefulWidget {
  const FaceVideoChildPage({Key? key, this.id = 0}) : super(key: key);
  final int id;

  @override
  State<FaceVideoChildPage> createState() => _FaceVideoChildPageState();
}

class _FaceVideoChildPageState extends State<FaceVideoChildPage> {
  bool isHud = true;
  bool netError = false;
  bool noMore = false;
  int page = 1;
  ...
  void initState() {
    ...
    super.initState();
    navs =
        Provider.of<BaseStore>(context, listen: false).conf?.video_sort ?? [];
    getData();
  }
  ...
  void uploadData(dynamic e, String url, int w, int h) {
    ...
    int remainFaceValue =
        Provider.of<BaseStore>(context, listen: false).user?.video_face_value ??
            0;
